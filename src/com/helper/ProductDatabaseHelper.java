...
public class ProductDatabaseHelper {
  ...
  public static ObservableList<Product> getAllProduct() {
    ...
    try (Connection cnt = DatabaseHelper.getConnetion();
      ...
      while (rs.next()) {
        ...
        list.add(new Product(id, categoryId, code, name, status, warrantyPeriod, importPrice, price,
            hardDrive, origin, imgSrc, screen, cpu, gpu, ram, operatingSystem, rearCamera,
    ...
    try (Connection cnt = DatabaseHelper.getConnetion();
      ...
      while (rs.next()) {
        ...
        if (status.equalsIgnoreCase(Main.UNLOCK)) {
          list.add(
              new Product(id, categoryId, code, name, status, warrantyPeriod, importPrice, price,
                  hardDrive, origin, imgSrc, screen, cpu, gpu, ram, operatingSystem, rearCamera,
                  selfieCamera, batteryCapacity, sim, weight, dimensions));
        }
      }
  ...
  public static Product getProductByName(String name) {
    ...
    try (Connection cnt = DatabaseHelper.getConnetion();
      ...
      while (rs.next()) {
        ...
        return new Product(id, categoryId, code, Name, status, warrantyPeriod, importPrice, price,
            hardDrive, origin, imgSrc, screen, cpu, gpu, ram, operatingSystem, rearCamera,
            selfieCamera, batteryCapacity, sim, weight, dimensions);
      }
    } catch (SQLException throwables) {
  ...
  public static Product getProductByCode(String name) {
    String query = "SELECT p.id, p.code, p.name AS 'product_name', p.status, p.warranty_period, p.import_price, p.price, p.hard_drive, p.origin, p.img_src, p.screen, p.cpu, p.gpu, p.ram, p.operating_system, p.rear_camera, p.selfie_camera, p.battery_capacity, p.sim, p.weight, p.dimensions, c.name AS 'category_name' FROM product AS p INNER JOIN categories AS c ON p.category_id=c.id WHERE p.code = ?;";
  ...
  public static ObservableList<Product> getAllProductByCategoryAndName(String category,
      String name) {
    ObservableList<Product> list = FXCollections.observableArrayList();
    String query =
        "SELECT p.id, p.code, p.name AS 'product_name', p.status, p.warranty_period, p.import_price, p.price, p.hard_drive, p.origin, "
            + "p.img_src, p.screen, p.cpu, p.gpu, p.ram, p.operating_system, p.rear_camera, p.selfie_camera, p.battery_capacity, p.sim, "
      ...
      while (rs.next()) {
        ...
        if (status.equalsIgnoreCase(Main.UNLOCK)) {
          list.add(
              new Product(id, categoryId, code, Name, status, warrantyPeriod, importPrice, price,
                  hardDrive,
                  origin, imgSrc, screen, cpu, gpu, ram, operatingSystem, rearCamera,
                  selfieCamera, batteryCapacity, sim, weight, dimensions));
        }
  ...
  public static boolean editProduct(Integer categoryId, String code, String name,
    ...
    String query = "UPDATE product SET category_id = ?, code = ?, name = ?, warranty_period = ?, import_price = ?, price = ?, hard_drive = ?, origin = ?, img_src = ?, screen = ?, cpu = ?, gpu = ?, ram = ?, operating_system = ?, rear_camera = ?, selfie_camera = ?, battery_capacity = ?, sim = ?, weight = ?, dimensions = ? WHERE id = ?";
